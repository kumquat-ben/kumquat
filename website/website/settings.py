...
NODE_LAUNCHER_KUBE_API_SERVER = env("NODE_LAUNCHER_KUBE_API_SERVER", "").strip()
NODE_LAUNCHER_KUBE_BEARER_TOKEN = env("NODE_LAUNCHER_KUBE_BEARER_TOKEN", "").strip()
NODE_LAUNCHER_KUBE_CA_CERT_B64 = env("NODE_LAUNCHER_KUBE_CA_CERT_B64", "").strip()
...
NODE_LAUNCHER_RUST_LOG = env(
    "NODE_LAUNCHER_RUST_LOG",
    "info,kumquat=debug",
).strip()
NODE_LAUNCHER_NETWORK = env("NODE_LAUNCHER_NETWORK", "dev")
NODE_LAUNCHER_CHAIN_ID = int(env("NODE_LAUNCHER_CHAIN_ID", "1337"))
NODE_LAUNCHER_HYBRID_ACTIVATION_HEIGHT = int(env("NODE_LAUNCHER_HYBRID_ACTIVATION_HEIGHT", "0"))
NODE_LAUNCHER_GENESIS_CEREMONY_FILE = env(
    "NODE_LAUNCHER_GENESIS_CEREMONY_FILE",
    str(BASE_DIR.parent / "blockchain" / "genesis.ceremony.json"),
).strip()
NODE_LAUNCHER_GENESIS_FILE = env(
    "NODE_LAUNCHER_GENESIS_FILE",
    str(BASE_DIR.parent / "blockchain" / "genesis.toml"),
).strip()
NODE_LAUNCHER_GENESIS_HASH = env(
    "NODE_LAUNCHER_GENESIS_HASH",
    "1b10e3582554ec4b197368743568f977db91110fd642c7f5c59ed17f83c9ca95",
).strip().lower()
...
NODE_LAUNCHER_GENESIS_SEED_HOST = env("NODE_LAUNCHER_GENESIS_SEED_HOST", "").strip().lower()
NODE_LAUNCHER_GENESIS_SEED_PORT = int(env("NODE_LAUNCHER_GENESIS_SEED_PORT", "30333"))
NODE_LAUNCHER_GENESIS_SEED_SERVICE_NAME = env(
    "NODE_LAUNCHER_GENESIS_SEED_SERVICE_NAME",
    "kumquat-blockchain-headless",
).strip()
...
EXPLORER_API_URL = env("EXPLORER_API_URL", "").rstrip("/")
...
SEARCH_CRAWLER_TIMEOUT_SECONDS = int(env("SEARCH_CRAWLER_TIMEOUT_SECONDS", "10"))
SEARCH_CRAWLER_USER_AGENT = env(
    "SEARCH_CRAWLER_USER_AGENT",
    "KumquatSearchBot/0.1 (+https://kumquat.info)",
)
OPENAI_SECRET_KEY = env("OPENAI_SECRET_KEY", "")
ENABLE_SCRAPER_SCHEDULER = env_bool("ENABLE_SCRAPER_SCHEDULER", False)
ENABLE_MANUAL_SCRIPT_SCHEDULER = env_bool("ENABLE_MANUAL_SCRIPT_SCHEDULER", False)
MANUAL_SCRIPT_TIMEOUT_SECONDS = int(env("MANUAL_SCRIPT_TIMEOUT_SECONDS", "600"))
MANUAL_SCRIPT_QUEUE_CONCURRENCY = int(env("MANUAL_SCRIPT_QUEUE_CONCURRENCY", "3"))
MANUAL_SCRIPT_QUEUE_POLL_SECONDS = int(env("MANUAL_SCRIPT_QUEUE_POLL_SECONDS", "2"))
