#!/usr/bin/env python3
"""Manual scraper for https://careers.fortive.com backed by Eightfold.ai.

The careers site is an Eightfold PCSx implementation. This script mirrors the
client-side flow: we page through `/api/pcsx/search` to collect position
summaries, then call `/api/pcsx/position_details` for rich fields (description,
apply URL, metadata). Results are written to the shared Django `JobPosting`
table so the operations dashboard can trigger or schedule this scraper.
"""
...
# Constants
# ---------------------------------------------------------------------------
BASE_URL = "https://fortive.eightfold.ai"
...
SCRAPER_QS = Scraper.objects.filter(company="Fortive", url=CAREERS_URL).order_by("id")
if SCRAPER_QS.exists():
    SCRAPER = SCRAPER_QS.first()
    if SCRAPER_QS.count() > 1:
        logging.getLogger(__name__).warning(
            "Multiple Fortive scrapers found; using id=%s.", SCRAPER.id
        )
...
class ScraperError(Exception):
    """Raised when the Fortive scraper encounters a fatal error."""
...
def ts_to_datestring(ts: Optional[int]) -> Optional[str]:
    if ts in (None, 0):
        return None
    try:
        # Eightfold returns epoch seconds.
...
class FortiveCareersClient:
    ...
    def iter_positions(self, *, start: int = 0, limit: Optional[int] = None) -> Iterator[JobSummary]:
        ...
        while True:
            ...
            if total_count is None:
                ...
                self.logger.info("Fortive API reports %s open jobs.", total_count)
    ...
    def _build_summary(self, raw: Dict[str, object]) -> JobSummary:
        ...
        locations = [loc for loc in raw.get("locations", []) if loc]
        ...
        summary = JobSummary(
            ...
            locations=locations,
            posted_ts=posted_ts,
            posted_date=ts_to_datestring(posted_ts),
            department=raw.get("department"),
            work_location_option=raw.get("workLocationOption"),
            ats_job_id=raw.get("atsJobId"),
            operating_companies=list(raw.get("efcustomTextOperatingcompany") or []),
        )
        return summary


class FortiveJobScraper:
    def __init__(self, *, client: Optional[FortiveCareersClient] = None) -> None:
        self.client = client or FortiveCareersClient()
        self.logger = logging.getLogger(self.__class__.__name__)
    ...
    def _build_listing(self, summary: JobSummary, detail: Dict[str, object]) -> JobListing:
        ...
        apply_action = (detail.get("positionUserActions") or {}).get("applyAction") or {}
        ...
        metadata: Dict[str, object] = {
            ...
            "operating_companies": summary.operating_companies,
...
def store_listing(listing: JobListing) -> bool:
    ...
    logging.getLogger("store_listing").debug(
        "Stored Fortive job '%s' (created=%s, id=%s)", obj.title, created, obj.id
    )
    return created


def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fortive careers manual scraper.")
...
def run_scrape(limit: Optional[int], start: int, delay: float, dry_run: bool) -> Dict[str, object]:
    client = FortiveCareersClient(delay=delay)
    scraper = FortiveJobScraper(client=client)
    ...
    try:
        for listing in scraper.scrape(limit=limit, start_offset=start):
            ...
            try:
                ...
                logging.error("Failed to store Fortive job %s: %s", listing.detail_url, exc)
                totals["errors"] += 1
    except requests.HTTPError as exc:
        logging.error("HTTP error while fetching Fortive jobs: %s", exc)
        totals["errors"] += 1
    except requests.RequestException as exc:
        logging.error("Network error while fetching Fortive jobs: %s", exc)
        totals["errors"] += 1
    except ScraperError as exc:
        logging.error("Fortive scraper stopped: %s", exc)
        totals["errors"] += 1
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    logging.info(
        "Fortive scraper finished - fetched=%(fetched)s created=%(created)s updated=%(updated)s errors=%(errors)s",
        totals,
    )
