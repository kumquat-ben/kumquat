#!/usr/bin/env python3
"""Manual scraper for C.H. Robinson (Workday-powered).

The public careers experience at https://www.chrobinson.com routes job searches
to the Workday tenant hosted at https://chrobinson.wd5.myworkdayjobs.com. This
script talks directly to that API, normalizes each posting, and persists the
results into ``scrapers.JobPosting`` via Django ORM.
"""
...
# Constants
# ---------------------------------------------------------------------------
CAREERS_URL = "https://www.chrobinson.com/en-us/about-us/careers/"
WORKDAY_ROOT = "https://chrobinson.wd5.myworkdayjobs.com"
TENANT = "chrobinson"
PORTAL = "CHRobinson"
...
SCRAPER_QS = Scraper.objects.filter(company="C.H. Robinson", url=CAREERS_URL).order_by("id")
if SCRAPER_QS.exists():
    SCRAPER = SCRAPER_QS.first()
    if SCRAPER_QS.count() > 1:
        logging.warning(
            "Multiple Scraper rows matched C.H. Robinson; using id=%s.", SCRAPER.id
        )
else:
...
class CHRobinsonJobScraper:
    """Client for interacting with the C.H. Robinson Workday job listings."""
        ...
        while True:
            ...
            try:
                ...
                if response.status_code == 422:
                    raise ScraperError(
                        "Workday jobs request returned HTTP 422. This usually means "
                        "a fresh Cloudflare/Workday session is required. "
                        f"Raw payload: {snippet}"
                    ) from exc
                raise ScraperError(
...
def persist_listing(listing: JobListing) -> bool:
    ...
    logging.getLogger("persist").debug(
        "Persisted C.H. Robinson job '%s' (created=%s, id=%s)", obj.title, created, obj.id
    )
    return created
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape C.H. Robinson Workday careers job listings."
    )
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    scraper = CHRobinsonJobScraper(page_size=args.page_size, delay=args.delay)
    ...
    logging.info(
        "C.H. Robinson scraper finished - fetched=%(fetched)s created=%(created)s "
