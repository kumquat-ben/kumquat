#!/usr/bin/env python3
"""Manual scraper for Digital Realty's Oracle Cloud Candidate Experience site."""
...
# Constants
# ---------------------------------------------------------------------------
CAREERS_LANDING_URL = "https://www.digitalrealty.com/about/careers"
ORACLE_BASE_URL = "https://hdep.fa.us2.oraclecloud.com"
SITE_CODE = "CX"
SITE_NUMBER = "CX"
...
SEARCH_PAGE_URL = (
    f"{ORACLE_BASE_URL}/hcmUI/CandidateExperience/en/sites/{SITE_CODE}/search"
)
...
SCRAPER_QS = Scraper.objects.filter(company="Digital Realty", url=CAREERS_LANDING_URL).order_by("id")
if SCRAPER_QS.exists():
    SCRAPER = SCRAPER_QS.first()
    if SCRAPER_QS.count() > 1:
        logging.warning("Multiple Digital Realty scraper rows found; using id=%s", SCRAPER.id)
else:
...
def _build_listing_metadata(summary: JobSummary, detail: dict) -> Dict[str, object]:
    ...
    if detail.get("workLocation"):
        metadata["work_location_raw"] = detail["workLocation"]
...
# ---------------------------------------------------------------------------
class DigitalRealtyCareersScraper:
    def __init__(
        self,
        *,
        ...
        while True:
            ...
            for summary in summaries:
                try:
                    ...
                    self.logger.error("Failed to fetch detail for job %s: %s", summary.job_id, exc)
                    continue
    ...
    def _parse_summary(self, raw: dict) -> Optional[JobSummary]:
        ...
        if raw.get("WorkplaceTypeCode"):
            metadata["workplace_type_code"] = raw.get("WorkplaceTypeCode")
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Digital Realty careers manual scraper")
    ...
    parser.add_argument("--page-size", type=int, default=DEFAULT_PAGE_SIZE, help="API page size (max 200)")
    parser.add_argument("--delay", type=float, default=DEFAULT_DELAY, help="Delay in seconds between job detail requests")
    parser.add_argument(
        "--log-level",
        type=str,
        default="INFO",
...
def run_scrape(args: argparse.Namespace) -> Dict[str, object]:
    scraper = DigitalRealtyCareersScraper(page_size=args.page_size, delay=args.delay)
    processed = 0
    created = 0
