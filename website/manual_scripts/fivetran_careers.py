#!/usr/bin/env python3
"""Manual scraper for https://fivetran.com/careers (Greenhouse-powered)."""
...
# Constants
# ---------------------------------------------------------------------------
CAREERS_URL = "https://fivetran.com/careers"
...
if SCRAPER_QS.exists():
    ...
    if SCRAPER_QS.count() > 1:
        logging.warning("Multiple Fivetran scraper rows found; using id=%s", SCRAPER.id)
else:
...
def _to_listing(raw: Dict[str, Any]) -> Optional[FivetranJob]:
    job_id = raw.get("id")
...
# ---------------------------------------------------------------------------
class FivetranGreenhouseScraper:
    def __init__(
        self,
        *,
    ...
    def scrape(self, *, limit: Optional[int] = None) -> Iterator[FivetranJob]:
        fetched = 0
        page = 1
...
# Persistence helpers
# ---------------------------------------------------------------------------
def persist_listing(listing: FivetranJob) -> bool:
    defaults = {
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scrape Fivetran careers listings.")
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    scraper = FivetranGreenhouseScraper(session=session, per_page=args.per_page, delay=args.delay)
    ...
    logging.info(
        "Fivetran scraper finished - fetched=%(fetched)s created=%(created)s "
