#!/usr/bin/env python3
"""Manual scraper for ConocoPhillips careers (Workday-powered)."""
...
# Constants
# ---------------------------------------------------------------------------
CAREERS_URL = "https://careers.conocophillips.com"
JOB_SEARCH_URL = "https://careers.conocophillips.com/job-search-results/"
WORKDAY_ROOT = "https://conocophillips.wd1.myworkdayjobs.com"
...
SCRAPER_QS = Scraper.objects.filter(company="ConocoPhillips", url=CAREERS_URL).order_by("id")
if SCRAPER_QS.exists():
    SCRAPER = SCRAPER_QS.first()
    if SCRAPER_QS.count() > 1:
        logging.warning("Multiple Scraper rows matched ConocoPhillips; using id=%s.", SCRAPER.id)
else:
...
class ScraperError(Exception):
    """Raised when the ConocoPhillips scrape encounters an unrecoverable error."""
...
class ConocoPhillipsJobScraper:
    def __init__(
        self,
        *,
    ...
    def _iter_summaries(self, *, limit: Optional[int]) -> Iterable[JobSummary]:
        ...
        while True:
            ...
            if response.status_code in {400, 403} and not self._bootstrapped:
                self.logger.info("Jobs endpoint returned %s; retrying after bootstrap.", response.status_code)
    ...
    def _fetch_detail_json(self, detail_path: str) -> Dict[str, object]:
        ...
        self.logger.debug("Fetching detail JSON %s", detail_url)
        response = self.session.get(detail_url, timeout=REQUEST_TIMEOUT)
        if response.status_code in {400, 403} and not self._bootstrapped:
            self.logger.info("Detail endpoint returned %s; attempting rebootstrap.", response.status_code)
            self._ensure_session_bootstrap(force=True)
            response = self.session.get(detail_url, timeout=REQUEST_TIMEOUT)
        if response.status_code == 404:
            raise ScraperError(f"Job detail endpoint returned 404 for {detail_path!r}")
        try:
            response.raise_for_status()
        except requests.HTTPError as exc:
            snippet = response.text[:200].strip()
    ...
    def _build_metadata(
        ...
        *,
        ...
        if isinstance(country, dict):
            country = country.get("descriptor") or country.get("id")
        metadata: Dict[str, object] = {
    ...
    def _ensure_session_bootstrap(self, force: bool = False) -> None:
        if self._bootstrapped and not force:
            return
        self.logger.debug("Bootstrapping Workday session.")
        resp = self.session.get(SESSION_SEED_URL, timeout=REQUEST_TIMEOUT)
        resp.raise_for_status()
        landing = self.session.get(JOB_SEARCH_URL, timeout=REQUEST_TIMEOUT)
...
def persist_listing(listing: JobListing) -> bool:
    ...
    logging.getLogger("persist").debug(
        "Persisted ConocoPhillips job '%s' (created=%s, id=%s)", obj.title, created, obj.id
    )
    return created


def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scrape ConocoPhillips careers listings.")
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    scraper = ConocoPhillipsJobScraper(page_size=args.page_size, delay=args.delay)
    ...
    logging.info(
        "ConocoPhillips scraper finished - fetched=%(fetched)s created=%(created)s updated=%(updated)s errors=%(errors)s",
        totals,
    )
    if totals["errors"]:
        return 1
