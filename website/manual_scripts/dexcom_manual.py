#!/usr/bin/env python3
"""Manual scraper for https://careers.dexcom.com/careers/index (Eightfold PCSx).

Dexcom's public careers experience runs on Eightfold's PCSx stack. The browser
retrieves paginated job summaries from `/api/pcsx/search` and then hydrates each
card via `/api/pcsx/position_details`. This script mirrors that flow and writes
results into the shared Django `JobPosting` table so operations can trigger the
scraper on demand.
"""
...
# Constants
# ---------------------------------------------------------------------------
BASE_URL = "https://careers.dexcom.com"
CAREERS_URL = f"{BASE_URL}/careers/index"
SEARCH_ENDPOINT = f"{BASE_URL}/api/pcsx/search"
DETAIL_ENDPOINT = f"{BASE_URL}/api/pcsx/position_details"
DEFAULT_DOMAIN = "dexcom.com"
PAGE_SIZE = 10
...
SCRAPER_QS = Scraper.objects.filter(company="Dexcom", url=CAREERS_URL).order_by("id")
if SCRAPER_QS.exists():
    SCRAPER = SCRAPER_QS.first()
    if SCRAPER_QS.count() > 1:
        logging.getLogger(__name__).warning(
            "Multiple Dexcom scrapers found; using id=%s.", SCRAPER.id
        )
...
class ScraperError(Exception):
    """Raised when the Dexcom scraper encounters a fatal error."""
...
class DexcomCareersClient:
    ...
    def iter_positions(self, *, start: int = 0, limit: Optional[int] = None) -> Iterator[JobSummary]:
        ...
        while True:
            ...
            if total_count is None:
                ...
                self.logger.info("Dexcom API reports %s open jobs.", total_count)
    ...
    def _build_summary(self, raw: Dict[str, object]) -> JobSummary:
        ...
        summary = JobSummary(
            ...
            standardized_locations=[loc for loc in raw.get("standardizedLocations", []) if loc],
...
class DexcomJobScraper:
    def __init__(self, *, client: Optional[DexcomCareersClient] = None) -> None:
        self.client = client or DexcomCareersClient()
        self.logger = logging.getLogger(self.__class__.__name__)
    ...
    def _build_listing(self, summary: JobSummary, detail: Dict[str, object]) -> JobListing:
        ...
        metadata: Dict[str, object] = {
            ...
            "jd_highlight": detail.get("jdHighlight"),
...
def store_listing(listing: JobListing) -> bool:
    ...
    logging.getLogger("store_listing").debug(
        "Stored Dexcom job '%s' (created=%s, id=%s)", obj.title, created, obj.id
    )
    return created


def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Dexcom careers manual scraper.")
...
def run_scrape(limit: Optional[int], start: int, delay: float, dry_run: bool) -> Dict[str, object]:
    client = DexcomCareersClient(delay=delay)
    scraper = DexcomJobScraper(client=client)
    ...
    try:
        for listing in scraper.scrape(limit=limit, start_offset=start):
            ...
            try:
                ...
                logging.error("Failed to store Dexcom job %s: %s", listing.detail_url, exc)
                totals["errors"] += 1
    except requests.HTTPError as exc:
        logging.error("HTTP error while fetching Dexcom jobs: %s", exc)
        totals["errors"] += 1
    except requests.RequestException as exc:
        logging.error("Network error while fetching Dexcom jobs: %s", exc)
        totals["errors"] += 1
    except ScraperError as exc:
        logging.error("Dexcom scraper stopped: %s", exc)
        totals["errors"] += 1
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    logging.info(
        "Dexcom scraper finished - fetched=%(fetched)s created=%(created)s updated=%(updated)s errors=%(errors)s",
        totals,
    )
