#!/usr/bin/env python3
"""Manual scraper for Camden Living (Workday-powered).

The public careers pages at https://www.camdenliving.com ultimately rely on
the Workday tenant hosted at https://camdenliving.wd1.myworkdayjobs.com. This
...
# Constants
# ---------------------------------------------------------------------------
CAREERS_URL = "https://www.camdenliving.com"
WORKDAY_ROOT = "https://camdenliving.wd1.myworkdayjobs.com"
TENANT = "camdenliving"
PORTAL = "CamdenCareers"
...
SCRAPER_QS = Scraper.objects.filter(company="Camden", url=CAREERS_URL).order_by("id")
if SCRAPER_QS.exists():
    SCRAPER = SCRAPER_QS.first()
    if SCRAPER_QS.count() > 1:
        logging.warning(
            "Multiple Scraper rows matched Camden; using id=%s.", SCRAPER.id
        )
else:
...
class CamdenJobScraper:
    """Client for interacting with the Camden Workday job listings."""
        ...
        while True:
            ...
            try:
                ...
                if response.status_code == 422:
                    raise ScraperError(
                        "Workday jobs request returned HTTP 422. Camden frequently "
                        "requires a solved Cloudflare session before the API responds. "
...
def persist_listing(listing: JobListing) -> bool:
    ...
    logging.getLogger("persist").debug(
        "Persisted Camden job '%s' (created=%s, id=%s)", obj.title, created, obj.id
    )
    return created
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape Camden Living Workday careers job listings."
    )
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    scraper = CamdenJobScraper(page_size=args.page_size, delay=args.delay)
    ...
    logging.info(
        "Camden scraper finished - fetched=%(fetched)s created=%(created)s "
