#!/usr/bin/env python3
"""Manual scraper for Church & Dwight careers (Workday-powered).

The public careers page at https://careers.churchdwight.com/jobs is backed by the
Church & Dwight Workday tenant (https://churchdwight.wd1.myworkdayjobs.com). This
script drives the same JSON APIs to enumerate job postings, enrich each summary
with data from the detail page JSON-LD payload, and upserts the results into the
shared ``JobPosting`` table.
"""
...
# Constants
# ---------------------------------------------------------------------------
CAREERS_URL = "https://careers.churchdwight.com/jobs"
WORKDAY_ROOT = "https://churchdwight.wd1.myworkdayjobs.com"
TENANT = "churchdwight"
PORTAL = "chdcareers"
...
SCRAPER_QS = Scraper.objects.filter(company="Church & Dwight", url=CAREERS_URL).order_by("id")
if SCRAPER_QS.exists():
    SCRAPER = SCRAPER_QS.first()
    if SCRAPER_QS.count() > 1:
        logging.warning(
            "Multiple Scraper rows matched Church & Dwight; using id=%s.", SCRAPER.id
        )
else:
...
class ScraperError(Exception):
    """Raised for unrecoverable errors while scraping Church & Dwight careers."""
...
class ChurchDwightWorkdayScraper:
    def __init__(
        self,
        *,
        page_size: int = 20,
    ...
    def _iter_summaries(self, *, limit: Optional[int]) -> Iterable[JobSummary]:
        ...
        while True:
            ...
            for raw in job_postings:
                ...
                if not detail_path:
                    self.logger.debug("Skipping job without a detail path: %s", raw)
                    continue
                detail_url = (
                    detail_path
                    if detail_path.startswith("http")
                    else urljoin(JOB_DETAIL_BASE.rstrip("/") + "/", detail_path.lstrip("/"))
                )
                summary = JobSummary(
                ...
                if not summary.title or not summary.detail_url:
                    self.logger.debug("Skipping incomplete job payload: %s", raw)
                    continue
                yield summary
                retrieved += 1
    ...
    def _extract_fallback_description(html_text: str) -> str:
        ...
        return ChurchDwightWorkdayScraper._clean_text(text) or ""

    @staticmethod
    def _normalize_description(raw: str) -> str:
        raw = ChurchDwightWorkdayScraper._clean_text(raw) or ""
        if not raw:
            return ""
...
def persist_listing(listing: JobListing) -> bool:
    ...
    logging.getLogger("persist").debug(
        "Persisted Church & Dwight job '%s' (created=%s, id=%s)", obj.title, created, obj.id
    )
    return created
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape Church & Dwight Workday careers job listings."
    )
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    scraper = ChurchDwightWorkdayScraper(page_size=args.page_size, delay=args.delay)
    ...
    logging.info(
        "Church & Dwight scraper finished - fetched=%(fetched)s created=%(created)s "
