#!/usr/bin/env python3
"""Manual scraper for https://www.checkr.com/background-check/employment-verification (Greenhouse-powered)."""
...
# Constants
# ---------------------------------------------------------------------------
CAREERS_URL = "https://www.checkr.com/background-check/employment-verification"
GREENHOUSE_BOARD = "checkr"
GREENHOUSE_API_URL = f"https://api.greenhouse.io/v1/boards/{GREENHOUSE_BOARD}/jobs"
COMPANY_NAME = "Checkr"
DEFAULT_HEADERS = {
    "User-Agent": (
...
if SCRAPER_QS.exists():
    ...
    if SCRAPER_QS.count() > 1:
        logging.warning("Multiple Checkr scraper rows found; using id=%s", SCRAPER.id)
else:
...
def _to_listing(raw: Dict[str, Any]) -> Optional[CheckrJob]:
    job_id = raw.get("id")
...
class CheckrGreenhouseScraper:
    ...
    def scrape(self, *, limit: Optional[int] = None) -> Iterator[CheckrJob]:
        fetched = 0
        page = 1
...
# Persistence helpers
# ---------------------------------------------------------------------------
def persist_listing(listing: CheckrJob) -> bool:
    defaults = {
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scrape Checkr job listings via Greenhouse.")
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    scraper = CheckrGreenhouseScraper(session=session, per_page=args.per_page, delay=args.delay)
    ...
    logging.info(
        "Checkr scraper finished - fetched=%(fetched)s created=%(created)s "
