#!/usr/bin/env python3
"""Manual scraper for Scale AI careers (Greenhouse-powered)."""
...
if SCRAPER_QS.exists():
    ...
    if SCRAPER_QS.count() > 1:
        logging.warning("Multiple Scale AI scraper rows found; using id=%s", SCRAPER.id)
else:
...
def _to_listing(raw: Dict[str, Any]) -> Optional[ScaleJob]:
    job_id = raw.get("id")
...
class ScaleGreenhouseScraper:
    ...
    def scrape(self, *, limit: Optional[int] = None) -> Iterator[ScaleJob]:
        fetched = 0
        page = 1
...
# Persistence helpers
# ---------------------------------------------------------------------------
def persist_listing(listing: ScaleJob) -> bool:
    defaults = {
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scrape Scale AI careers listings.")
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    scraper = ScaleGreenhouseScraper(session=session, per_page=args.per_page, delay=args.delay)
    ...
    logging.info(
        "Scale AI scraper finished - fetched=%(fetched)s created=%(created)s "
