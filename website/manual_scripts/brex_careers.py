#!/usr/bin/env python3
"""Manual scraper for https://www.brex.com/careers (Greenhouse-powered)."""
...
GREENHOUSE_API_URL = "https://api.greenhouse.io/v1/boards/brex/jobs"
COMPANY_NAME = "Brex"
DEFAULT_HEADERS = {
    "User-Agent": (
...
if SCRAPER_QS.exists():
    ...
    if SCRAPER_QS.count() > 1:
        logging.warning("Multiple Brex scraper rows found; using id=%s", SCRAPER.id)
else:
...
def _to_listing(raw: Dict[str, Any]) -> Optional[BrexJob]:
    job_id = raw.get("id")
...
class BrexGreenhouseScraper:
    ...
    def scrape(self, *, limit: Optional[int] = None) -> Iterator[BrexJob]:
        fetched = 0
        page = 1
...
# Persistence
# ---------------------------------------------------------------------------
def store_listing(listing: BrexJob) -> None:
...
def run_scrape(limit: Optional[int], delay: float) -> int:
    scraper = BrexGreenhouseScraper(delay=delay)
    count = 0
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Brex careers scraper")
