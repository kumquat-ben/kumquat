#!/usr/bin/env python3
"""Manual scraper for Chubb's Oracle Cloud Candidate Experience site."""
...
COMPANY_NAME = "Chubb"
CAREERS_LANDING_URL = (
    "https://fa-ewgu-saasfaprod1.fa.ocs.oraclecloud.com/"
    "hcmUI/CandidateExperience/en/sites/CX_2001"
)
ORACLE_BASE_URL = "https://fa-ewgu-saasfaprod1.fa.ocs.oraclecloud.com"
SITE_CODE = "CX_2001"
SITE_NUMBER = "2001"
...
SCRAPER_QS = Scraper.objects.filter(company=COMPANY_NAME, url=CAREERS_LANDING_URL).order_by("id")
if SCRAPER_QS.exists():
    SCRAPER = SCRAPER_QS.first()
    if SCRAPER_QS.count() > 1:
        logging.warning("Multiple Chubb scraper rows found; using id=%s", SCRAPER.id)
else:
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Chubb careers manual scraper")
...
def run_scrape(args: argparse.Namespace) -> Dict[str, object]:
    scraper = ChubbCareersScraper(page_size=args.page_size, delay=args.delay)
    processed = 0
    created = 0
