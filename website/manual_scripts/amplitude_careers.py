#!/usr/bin/env python3
"""Manual scraper for https://amplitude.com/careers (Greenhouse-powered)."""
...
GREENHOUSE_API_URL = "https://api.greenhouse.io/v1/boards/amplitude/jobs"
...
if SCRAPER_QS.exists():
    ...
    if SCRAPER_QS.count() > 1:
        logging.warning("Multiple Amplitude scraper rows found; using id=%s", SCRAPER.id)
else:
...
def _to_listing(raw: Dict[str, Any]) -> Optional[AmplitudeJob]:
    job_id = raw.get("id")
...
# ---------------------------------------------------------------------------
class AmplitudeGreenhouseScraper:
    def __init__(
        self,
        *,
    ...
    def scrape(self, *, limit: Optional[int] = None) -> Iterator[AmplitudeJob]:
        fetched = 0
        page = 1
...
# Persistence
# ---------------------------------------------------------------------------
def store_listing(listing: AmplitudeJob) -> None:
...
def run_scrape(limit: Optional[int], delay: float) -> int:
    scraper = AmplitudeGreenhouseScraper(delay=delay)
    count = 0
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Amplitude careers scraper")
