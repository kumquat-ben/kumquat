#!/usr/bin/env python3
"""Manual scraper for Air Products' Workday careers site."""
...
# ---------------------------------------------------------------------------
CAREERS_URL = "https://airproducts.wd5.myworkdayjobs.com/AP0001"
WORKDAY_ROOT = "https://airproducts.wd5.myworkdayjobs.com"
TENANT = "airproducts"
PORTAL = "AP0001"
...
SCRAPER_QS = Scraper.objects.filter(company="Air Products", url=CAREERS_URL).order_by("id")
if SCRAPER_QS.exists():
    SCRAPER = SCRAPER_QS.first()
    if SCRAPER_QS.count() > 1:
        logging.warning("Multiple Scraper rows matched Air Products; using id=%s.", SCRAPER.id)
else:
...
class ScraperError(Exception):
    """Raised when the Air Products scraper cannot continue."""
...
class AirProductsJobScraper:
    ...
    def _store_csrf_token(self, html: str) -> None:
        ...
        if not match:
            logging.warning("Failed to locate CSRF token in Workday bootstrap page; subsequent requests may fail.")
            return
        ...
        if token != self._csrf_token:
            logging.debug("Updated Workday CSRF token.")
...
def persist_listing(listing: JobListing) -> bool:
    ...
    logging.getLogger("persist").debug(
        "Persisted Air Products job '%s' (created=%s, id=%s)", obj.title, created, obj.id
    )
    return created


def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scrape Air Products Workday listings.")
    parser.add_argument("--limit", type=int, default=None, help="Maximum jobs to process.")
    parser.add_argument(
        "--page-size",
        type=int,
        default=50,
        help="Number of records requested per page (default: 50).",
    )
    parser.add_argument(
        "--delay",
        type=float,
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    scraper = AirProductsJobScraper(page_size=args.page_size, delay=args.delay)
    ...
    logging.info(
        "Air Products scraper finished - fetched=%(fetched)s created=%(created)s",
        totals,
    )
    return 0
