#!/usr/bin/env python3
"""Manual scraper for Cummins careers (cummins.com/careers).

Cummins' public careers microsite (https://cummins.jobs/) renders job listings
client-side by querying the Jobsyn Solr API. This script mirrors that behaviour:
it pages through the API, normalises each listing, and persists the results via
the shared Django models so the manual runner can execute it like the other
manual scrapers.
"""
...
# Constants
# ---------------------------------------------------------------------------
CAREERS_LANDING_URL = "https://www.cummins.com/careers"
JOB_SEARCH_URL = "https://cummins.jobs/jobs/"
SITE_BASE_URL = "https://cummins.jobs"
...
SCRAPER_QS = Scraper.objects.filter(company="Cummins", url=CAREERS_LANDING_URL).order_by("id")
if SCRAPER_QS.exists():
    SCRAPER = SCRAPER_QS.first()
    if SCRAPER_QS.count() > 1:
        logging.getLogger(__name__).warning(
            "Multiple Scraper rows matched Cummins careers; using id=%s", SCRAPER.id
        )
...
class ScraperError(Exception):
    """Raised when the Cummins scraper encounters a non-recoverable error."""
...
def slugify(value: str) -> str:
    """Produce a slug similar to the Jobsyn front-end implementation."""
    if not value:
        return ""
...
class CumminsJobScraper:
    ...
    def scrape(
        ...
        *,
        ...
        while True:
            ...
            payload = self._fetch_page(page=page)
            jobs = payload.get("jobs") or []
            if not jobs:
                self.logger.info("API returned no jobs at page %s; stopping.", page)
                break
    ...
    def _transform(self, job: Dict[str, object]) -> JobListing:
        ...
        location_text = str(job.get("location_exact") or "") or None
        location_slug = slugify(location_text or "")
        if not location_slug:
            city_slug = slugify(str(job.get("city_exact") or ""))
            state = str(job.get("state_short") or "").lower()
            parts = [part for part in (city_slug, state) if part]
        ...
        link = urljoin(SITE_BASE_URL, relative_path)
        ...
        return JobListing(
            guid=guid,
            reqid=str(job.get("reqid") or "").strip() or None,
            title=title,
...
def persist_listing(listing: JobListing) -> bool:
    ...
    logging.getLogger("persist").debug(
        "Persisted Cummins job '%s' (created=%s, id=%s)", obj.title, created, obj.id
    )
    return created
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scrape Cummins job listings.")
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    scraper = CumminsJobScraper(delay=args.delay)
    ...
    try:
        ...
        logging.error("Cummins scraper stopped due to error: %s", exc)
        totals["errors"] += 1
    ...
    logging.info(
        "Cummins scraper finished - fetched=%(fetched)s created=%(created)s "
