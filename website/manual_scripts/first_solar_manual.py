#!/usr/bin/env python3
"""Manual scraper for First Solar's Oracle Cloud Candidate Experience site."""
...
# Constants
# ---------------------------------------------------------------------------
CAREERS_LANDING_URL = "https://www.firstsolar.com/en/Careers"
ORACLE_BASE_URL = "https://fa-esbv-saasfaprod1.fa.ocs.oraclecloud.com"
SITE_CODE = "CX_1"
SITE_NUMBER = "1"
...
SCRAPER_QS = Scraper.objects.filter(company="First Solar", url=CAREERS_LANDING_URL).order_by("id")
if SCRAPER_QS.exists():
    SCRAPER = SCRAPER_QS.first()
    if SCRAPER_QS.count() > 1:
        logging.warning("Multiple First Solar scraper rows found; using id=%s", SCRAPER.id)
else:
...
# ---------------------------------------------------------------------------
class FirstSolarCareersScraper:
    def __init__(
        self,
        *,
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="First Solar careers manual scraper")
...
def run_scrape(args: argparse.Namespace) -> Dict[str, object]:
    scraper = FirstSolarCareersScraper(page_size=args.page_size, delay=args.delay)
    processed = 0
    created = 0
