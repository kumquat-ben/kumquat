#!/usr/bin/env python3
"""Manual scraper for Target Corporate Careers (Workday-powered).

This script iterates through Target's public Workday job listings API,
enriches each job with metadata from its detail page JSON-LD payload,
and persists the results into the existing ``JobPosting`` table that is
linked to the "Target" scraper entry.
"""
...
WORKDAY_ROOT = "https://target.wd5.myworkdayjobs.com"
PORTAL = "targetcareers"
CXS_BASE = f"{WORKDAY_ROOT}/wday/cxs/target/{PORTAL}"
...
SCRAPER_QS = Scraper.objects.filter(company="Target", url=CAREERS_URL).order_by("id")
if SCRAPER_QS.exists():
    SCRAPER = SCRAPER_QS.first()
    if SCRAPER_QS.count() > 1:
        logging.warning("Multiple Scraper rows matched Target careers; using id=%s", SCRAPER.id)
else:
...
class ScraperError(Exception):
    """Raised for unrecoverable errors while scraping Target careers."""
...
def persist_listing(listing: JobListing) -> bool:
    ...
    logging.getLogger("persist").debug(
        "Persisted Target job '%s' (created=%s, id=%s)", obj.title, created, obj.id
    )
    return created


def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scrape Target corporate careers job listings.")
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    scraper = TargetJobScraper(page_size=args.page_size, delay=args.delay)
    ...
    logging.info(
        "Target scraper finished - fetched=%(fetched)s created=%(created)s "
