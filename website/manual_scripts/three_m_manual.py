#!/usr/bin/env python3
"""Manual scraper for 3M careers (Workday-powered).

The public careers page at https://www.3m.com/3M/en_US/careers-us delegates job
search to a Workday tenant (https://3m.wd1.myworkdayjobs.com). This script
drives the same JSON APIs used by the website to enumerate job postings,
enriches each posting with metadata from its detail page JSON-LD payload, and
persists the results into the shared ``JobPosting`` table.
"""
...
# Constants
# ---------------------------------------------------------------------------
CAREERS_URL = "https://www.3m.com/3M/en_US/careers-us"
WORKDAY_ROOT = "https://3m.wd1.myworkdayjobs.com"
TENANT = "3m"
PORTAL = "Search"
...
SCRAPER_QS = Scraper.objects.filter(company="3M", url=CAREERS_URL).order_by("id")
if SCRAPER_QS.exists():
    SCRAPER = SCRAPER_QS.first()
    if SCRAPER_QS.count() > 1:
        logging.warning("Multiple Scraper rows matched 3M; using id=%s.", SCRAPER.id)
else:
...
class ScraperError(Exception):
    """Raised for unrecoverable errors while scraping 3M careers."""
...
class ThreeMJobScraper:
    ...
    def _extract_fallback_description(html_text: str) -> str:
        ...
        return ThreeMJobScraper._clean_text(text) or ""

    @staticmethod
    def _normalize_description(raw: str) -> str:
        raw = ThreeMJobScraper._clean_text(raw) or ""
        if not raw:
            return ""
...
def persist_listing(listing: JobListing) -> bool:
    ...
    logging.getLogger("persist").debug(
        "Persisted 3M job '%s' (created=%s, id=%s)", obj.title, created, obj.id
    )
    return created


def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scrape 3M Workday careers job listings.")
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    scraper = ThreeMJobScraper(page_size=args.page_size, delay=args.delay)
    ...
    logging.info(
        "3M scraper finished - fetched=%(fetched)s created=%(created)s "
