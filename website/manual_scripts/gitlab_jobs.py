#!/usr/bin/env python3
"""Manual scraper for GitLab jobs (Greenhouse-powered)."""
...
# Constants
# ---------------------------------------------------------------------------
CAREERS_URL = "http://gitlab.com/jobs/"
...
if SCRAPER_QS.exists():
    ...
    if SCRAPER_QS.count() > 1:
        logging.warning("Multiple GitLab scraper rows found; using id=%s", SCRAPER.id)
else:
...
def _to_listing(raw: Dict[str, Any]) -> Optional[GitLabJob]:
    job_id = raw.get("id")
...
class GitLabGreenhouseScraper:
    ...
    def scrape(self, *, limit: Optional[int] = None) -> Iterator[GitLabJob]:
        fetched = 0
        page = 1
...
# Persistence helpers
# ---------------------------------------------------------------------------
def persist_listing(listing: GitLabJob) -> bool:
    defaults = {
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scrape GitLab job listings.")
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    scraper = GitLabGreenhouseScraper(session=session, per_page=args.per_page, delay=args.delay)
    ...
    logging.info(
        "GitLab scraper finished - fetched=%(fetched)s created=%(created)s "
