#!/usr/bin/env python3
"""Manual scraper for https://webflow.com/company/careers (Greenhouse-powered)."""
...
# Constants
# ---------------------------------------------------------------------------
CAREERS_URL = "https://webflow.com/company/careers"
GREENHOUSE_API_URL = "https://boards-api.greenhouse.io/v1/boards/webflow/jobs"
...
if SCRAPER_QS.exists():
    ...
    if SCRAPER_QS.count() > 1:
        logging.warning("Multiple Webflow scraper rows found; using id=%s", SCRAPER.id)
else:
...
def _to_listing(raw: Dict[str, Any]) -> Optional[WebflowJob]:
    job_id = raw.get("id")
...
# ---------------------------------------------------------------------------
class WebflowGreenhouseScraper:
    def __init__(
        self,
        *,
    ...
    def scrape(self, *, limit: Optional[int] = None) -> Iterator[WebflowJob]:
        fetched = 0
        page = 1
...
# Persistence
# ---------------------------------------------------------------------------
def store_listing(listing: WebflowJob) -> None:
...
def run_scrape(limit: Optional[int], delay: float) -> int:
    scraper = WebflowGreenhouseScraper(delay=delay)
    count = 0
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Webflow careers scraper")
