#!/usr/bin/env python3
"""Manual scraper for https://benchling.com/careers (Greenhouse-powered)."""
...
# Constants
# ---------------------------------------------------------------------------
CAREERS_URL = "https://benchling.com/careers"
GREENHOUSE_BOARD = "benchling"
...
if SCRAPER_QS.exists():
    ...
    if SCRAPER_QS.count() > 1:
        logging.warning("Multiple Benchling scraper rows found; using id=%s", SCRAPER.id)
else:
...
def _to_listing(raw: Dict[str, Any]) -> Optional[BenchlingJob]:
    job_id = raw.get("id")
...
# ---------------------------------------------------------------------------
class BenchlingGreenhouseScraper:
    def __init__(
        self,
        *,
    ...
    def scrape(self, *, limit: Optional[int] = None) -> Iterator[BenchlingJob]:
        fetched = 0
        page = 1
...
# Persistence helpers
# ---------------------------------------------------------------------------
def persist_listing(listing: BenchlingJob) -> bool:
    defaults = {
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scrape Benchling careers listings.")
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    scraper = BenchlingGreenhouseScraper(session=session, per_page=args.per_page, delay=args.delay)
    ...
    logging.info(
        "Benchling scraper finished - fetched=%(fetched)s created=%(created)s "
