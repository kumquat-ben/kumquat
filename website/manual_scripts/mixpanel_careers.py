#!/usr/bin/env python3
"""Manual scraper for https://mixpanel.com/jobs (Greenhouse-powered)."""
...
if SCRAPER_QS.exists():
    ...
    if SCRAPER_QS.count() > 1:
        logging.warning("Multiple Mixpanel scraper rows found; using id=%s", SCRAPER.id)
else:
...
def _to_listing(raw: Dict[str, Any]) -> Optional[MixpanelJob]:
    job_id = raw.get("id")
...
# ---------------------------------------------------------------------------
class MixpanelGreenhouseScraper:
    def __init__(
        self,
        *,
    ...
    def scrape(self, *, limit: Optional[int] = None) -> Iterator[MixpanelJob]:
        fetched = 0
        page = 1
...
# Persistence helpers
# ---------------------------------------------------------------------------
def persist_listing(listing: MixpanelJob) -> bool:
    defaults = {
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scrape Mixpanel careers listings.")
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    scraper = MixpanelGreenhouseScraper(session=session, per_page=args.per_page, delay=args.delay)
    ...
    logging.info(
        "Mixpanel scraper finished - fetched=%(fetched)s created=%(created)s "
