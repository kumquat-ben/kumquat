#!/usr/bin/env python3
"""Manual scraper for Cincinnati Insurance's Taleo-powered careers site."""
...
# Constants
# ---------------------------------------------------------------------------
CAREERS_URL = "https://www.cinfin.com/cincinnati-insurance-careers/openings"
TALEO_BASE = "https://cinfin.taleo.net"
PORTAL_ID = "101430233"
SEARCH_ENDPOINT = f"{TALEO_BASE}/careersection/rest/jobboard/searchjobs?lang=en&portal={PORTAL_ID}"
JOB_DETAIL_BASE = f"{TALEO_BASE}/careersection/ex/jobdetail.ftl"
DEFAULT_HEADERS = {
    "User-Agent": (
...
SCRAPER_QS = Scraper.objects.filter(company="Cincinnati Insurance", url=CAREERS_URL).order_by("id")
if SCRAPER_QS.exists():
    SCRAPER = SCRAPER_QS.first()
    if SCRAPER_QS.count() > 1:
        logging.warning("Multiple Scraper rows matched Cincinnati Insurance; using id=%s.", SCRAPER.id)
else:
...
# ---------------------------------------------------------------------------
class CincinnatiInsuranceJobScraper:
    def __init__(
        self,
        *,
        delay: float = 0.25,
    ...
    def _fetch_jobs_page(self, page_no: int) -> dict:
        ...
        self.logger.debug("Fetching Cincinnati Insurance jobs page %s", page_no)
    ...
    def _parse_summary(self, raw: dict) -> Optional[JobSummary]:
        ...
        title = (columns[1] if len(columns) > 1 else columns[0] or "").strip()
        location = self._parse_location(columns[2] if len(columns) > 2 else None)
        date_posted = None  # Taleo JSON does not include posting date in column payload for this site.
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Cincinnati Insurance manual scraper.")
...
def run_scrape(
    *,
    ...
    scraper = CincinnatiInsuranceJobScraper(delay=delay)
    ...
    try:
        ...
        logging.error("HTTP error while scraping Cincinnati Insurance: %s", exc)
        totals["errors"] += 1
    except requests.RequestException as exc:
        logging.error("Network error while scraping Cincinnati Insurance: %s", exc)
        totals["errors"] += 1
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    logging.info(
        "Cincinnati Insurance scraper finished - fetched=%(fetched)s created=%(created)s "
