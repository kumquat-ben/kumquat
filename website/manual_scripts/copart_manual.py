#!/usr/bin/env python3
"""Manual scraper for Copart careers (Workday-powered).

This script fetches Copart job listings from the public Workday API, enriches
each posting with metadata from the associated detail page, and persists the
results into the existing ``JobPosting`` table that is linked to the "Copart"
scraper entry.
"""
...
# Constants
# ---------------------------------------------------------------------------
CAREERS_URL = "https://copart.com/careers"
WORKDAY_ROOT = "https://copart.wd12.myworkdayjobs.com"
COMPANY_SLUG = "copart"
PORTAL = "Copart"
CXS_BASE = f"{WORKDAY_ROOT}/wday/cxs/{COMPANY_SLUG}/{PORTAL}"
...
SCRAPER_QS = Scraper.objects.filter(company="Copart", url=CAREERS_URL).order_by("id")
if SCRAPER_QS.exists():
    SCRAPER = SCRAPER_QS.first()
    if SCRAPER_QS.count() > 1:
        logging.warning("Multiple Scraper rows matched Copart careers; using id=%s", SCRAPER.id)
else:
...
class ScraperError(Exception):
    """Raised for unrecoverable errors while scraping Copart careers."""
...
class CopartJobScraper:
            ...
            for raw in job_postings:
                ...
                if detail_path:
                    detail_url = (
                        ...
                        else f"{JOB_DETAIL_BASE.rstrip('/')}{detail_path}"
                    )
                summary = JobSummary(
    ...
    def _enrich_summary(self, summary: JobSummary) -> JobListing:
        ...
        metadata = self._build_metadata(summary, json_ld)
    ...
    def _extract_json_ld(self, html_text: str) -> Dict[str, Any]:
    ...
    def _build_metadata(self, summary: JobSummary, json_ld: Dict[str, Any]) -> Dict[str, Any]:
        metadata: Dict[str, Any] = {
        ...
        if isinstance(json_ld, dict):
            metadata["employment_type"] = json_ld.get("employmentType")
            metadata["hiring_organization"] = json_ld.get("hiringOrganization")
            metadata["job_location"] = self._extract_locations(json_ld.get("jobLocation"))
            metadata["json_ld"] = json_ld
        return metadata

    @staticmethod
    def _extract_locations(raw_location: Any) -> Optional[List[Dict[str, Any]]]:
        ...
        for entry in raw_location:
            if not isinstance(entry, dict):
                continue
            cleaned: Dict[str, Any] = {}
            for key in ("@type", "name", "address"):
                if key in entry:
...
def persist_listing(listing: JobListing) -> bool:
    ...
    logging.getLogger("persist").debug(
        "Persisted Copart job '%s' (created=%s, id=%s)", obj.title, created, obj.id
    )
    return created


def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scrape Copart Workday job listings.")
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    scraper = CopartJobScraper(page_size=args.page_size, delay=args.delay)
    ...
    logging.info(
        "Copart scraper finished - fetched=%(fetched)s created=%(created)s "
