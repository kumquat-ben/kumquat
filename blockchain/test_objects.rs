...
fn main() {
    ...
    nft_metadata.insert(
        ...
        "A unique digital collectible".to_string(),
    );
    nft_metadata.insert(
        "image".to_string(),
        "https://example.com/nft1.png".to_string(),
    );
    ...
    let shared_object = object_store
        .create_object(
            Ownership::Shared,
    ...
    let immutable_object = object_store
        .create_object(
            Ownership::Immutable,
    ...
    let owner_objects = object_store
        .get_objects_by_owner(&owner_address)
    ...
    // Get objects by type
    let coins = object_store
        .get_objects_by_type("Coin")
    ...
    if let Some(coin) = coins.first() {
        let updated_coin = object_store
            .update_object(&coin.id, |obj| {
    ...
    if let Some(nft) = object_store
        .get_objects_by_type("NFT")
        .expect("Failed to get NFTs")
        .first()
    {
        let transferred_nft = object_store
            .update_object(&nft.id, |obj| {
    ...
    let recipient_objects = object_store
        .get_objects_by_owner(&recipient)
        .expect("Failed to get objects by recipient");

    println!("Objects owned by recipient {}:", encode_address(&recipient));
    ...
    if let Some(coin) = coins.first() {
        object_store
            .delete_object(&coin.id)
            .expect("Failed to delete coin object");
        println!("Deleted coin object: {}", coin.id_hex());
    }

    // Verify the coin is deleted
    let remaining_coins = object_store
        .get_objects_by_type("Coin")
        .expect("Failed to get objects by type");
    ...
    let all_objects = object_store
        .get_all_objects()
        .expect("Failed to get all objects");
