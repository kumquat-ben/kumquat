...
impl BlockTemplate {
    ...
    pub fn to_block(&self, nonce: u64, poh_hash: [u8; 32]) -> Block {
        ...
        // Use the provided tx_root or calculate it
        let tx_root = self.tx_root.unwrap_or_else(|| self.calculate_tx_root());
        ...
        Block {
            ...
            tx_root,
            nonce,
            poh_seq: self.poh_sequence_start,
            poh_hash,
            difficulty: self.target.to_difficulty(),
            total_difficulty: 0, // This will be set by the miner
        }
    }

    /// Calculate the transaction root (Merkle root of transactions)
    fn calculate_tx_root(&self) -> Hash {
        // Simple Merkle root calculation
        if self.transactions.is_empty() {
            return [0u8; 32];
        }

        let mut hashes = self.transactions.clone();
