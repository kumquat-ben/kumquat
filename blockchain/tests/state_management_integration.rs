...
use kumquat::storage::{
    AccountState, AccountType, BlockStore, IndexConfig, IndexType, PrunerConfig, PruningMode,
    PruningResult, RocksDBStore, ShardConfig, ShardingStrategy, StateIndexingManager, StatePruner,
    StateRoot, StateShardingManager, StateStore, StateValidator, ValidationError, ValidationResult,
};
...
fn test_state_management_integration() {
    ...
    let sharding_manager =
        StateShardingManager::new(&kv_store, &state_store, &block_store, sharding_config);
    ...
    for i in 0..10 {
        ...
        if let Some(account) = state_store.get_account(&address, height).unwrap() {
            indexing_manager
                .index_account(&address, &account, height)
                .unwrap();
        }
    }
