...
use kumquat::storage::{
    AccountState, AccountType, Block, BlockStore, IndexConfig, IndexType, IndexingStatus,
    PrunerConfig, PruningError, PruningMode, PruningResult, RocksDBStore, ShardConfig,
    ShardingError, ShardingStrategy, StateIndexingManager, StateManager, StatePruner, StateRoot,
    StateShardingManager, StateStore, StateSynchronizer, StateValidator, SyncConfig, SyncMode,
    SyncStatus, ValidationError, ValidationResult,
};
...
fn test_state_validation() {
    ...
    state_store
        .put_account(&address1, &account1, 100)
        .unwrap()
        .unwrap();
    ...
    state_store
        .put_account(&address2, &account2, 100)
        .unwrap()
        .unwrap();
...
fn test_state_indexing() {
    ...
    manager
        .create_index("balance_index", balance_config)
        .unwrap();
    ...
    manager
        .create_index("account_type_index", account_type_config)
        .unwrap();
    ...
    for i in 0u8..10u8 {
        ...
        // Store the account
        state_store
            .put_account(&address, &account, height)
            .unwrap()
            .unwrap();
    ...
    let results = balance_index
        .query_by_balance_range(3000, 5000, 10, None)
        .unwrap();
    ...
    let results = account_type_index
        .query_by_account_type("User", 10, None)
        .unwrap();
    assert_eq!(results.len(), 4); // 4 user accounts (i = 0, 3, 6, 9)

    let results = account_type_index
        .query_by_account_type("Contract", 10, None)
        .unwrap();
    assert_eq!(results.len(), 3); // 3 contract accounts (i = 1, 4, 7)

    let results = account_type_index
        .query_by_account_type("Validator", 10, None)
        .unwrap();
