...
impl SystemRouter {
    ...
    async fn register_transaction_handler(&self) -> Result<(), String> {
        ...
        if let Some(mempool) = mempool {
            ...
            router
                .register_handler("new_transaction", move |node_id, message| {
                    if let NetMessage::NewTransaction(tx) = message {
                        ...
                        let mempool_tx = crate::mempool::types::TransactionRecord {
                            ...
                            fee_token_id: None,
                            coin_transfer: tx_clone.coin_transfer.clone(),
                            coin_fee: tx_clone.coin_fee.clone(),
                            ...
                            match mempool_clone.insert(mempool_tx).await {
                                Ok(_) => {
                                    ...
                                    if let Some(broadcaster) = broadcaster_clone {
                                        broadcaster
                                            .broadcast(NetMessage::NewTransaction(tx_clone))
                                            .await;
                                    }
                                }
    ...
    async fn register_block_handler(&self) -> Result<(), String> {
        ...
        if let Some(block_store) = block_store {
            ...
            router
                .register_handler("new_block", move |node_id, message| {
                    if let NetMessage::NewBlock(block) = message {
                        ...
                        tokio::spawn(async move {
                            ...
                            if let Some(consensus) = consensus_clone {
                                ...
                                if let Some(broadcaster) = broadcaster_clone {
                                    broadcaster
                                        .broadcast(NetMessage::NewBlock(block_clone))
                                        .await;
                                }
                            }
                        });
    ...
    async fn register_block_request_handler(&self) -> Result<(), String> {
        ...
        if let Some(block_store) = block_store {
            ...
            router
                .register_handler("request_block", move |node_id, message| {
                            ...
                            if let Some(ref _broadcaster) = broadcaster_clone {
                                ...
                                if let Some(ref broadcaster) = broadcaster_clone {
                                    match broadcaster.send_to_peer(&node_id_clone, response).await {
                                        Ok(_) => {}
                                        Err(e) => error!(
                                            "Failed to send block response to {}: {}",
                                            node_id_clone, e
                                        ),
                                    }
                                }
    ...
    async fn register_block_range_request_handler(&self) -> Result<(), String> {
        ...
        if let Some(block_store) = block_store {
            ...
            router
                .register_handler("request_block_range", move |node_id, message| {
                    if let NetMessage::RequestBlockRange {
                        start_height,
                        end_height,
                    } = message
                    {
                        debug!(
                            "Received block range request from {}: {}..{}",
                            node_id, start_height, end_height
                        );
                        ...
                        tokio::task::spawn(async move {
                            ...
                            if let Some(broadcaster) = broadcaster_clone {
                                ...
                                match broadcaster.send_to_peer(&node_id_clone, response).await {
                                    Ok(_) => {}
                                    Err(e) => error!(
                                        "Failed to send block range response to {}: {}",
                                        node_id_clone, e
                                    ),
                                }
                            }
