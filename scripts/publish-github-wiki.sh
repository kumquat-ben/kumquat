...
TMP_DIR="$(mktemp -d)"
REMOTE_URL="https://x-access-token:${GITHUB_TOKEN:?GITHUB_TOKEN is required}@github.com/kumquat-ben/kumquat.wiki.git"
...
git clone "$REMOTE_URL" "$TMP_DIR/wiki" >/dev/null 2>&1 || {
  ...
  echo "If the wiki has not been initialized yet, open https://github.com/kumquat-ben/kumquat/wiki and create the first page once."
  exit 1
}
